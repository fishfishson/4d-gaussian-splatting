...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, scale_mod):
    ...
    cmd = ['ffmpeg', '-y', '-framerate', '30', '-i', os.path.join(render_path, '%05d.jpg'), '-c:v', 'libx264', '-pix_fmt', 'yuv420p', os.path.join(render_path, f'../render-{scale_mod}.mp4')]
    subprocess.run(cmd)

def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, time_duration: list, scale_mod: float):
    with torch.no_grad():
...
if __name__ == "__main__":
    ...
    parser.add_argument("--time_duration", default=[0, 1], nargs=2, type=float, help="Time duration for the rendering")
    ...
    render_sets(model.extract(args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_test, args.time_duration, args.scale_mod)